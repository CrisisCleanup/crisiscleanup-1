...
def get_form_data_types(event):
    q = db.Query(form_types_db.FormTypes)
    q.filter("incident =", event.key())
    query = q.get()

    properties_json = query.properties_json
...
def write_csv_template(fd, event):
    writer = UnicodeWriter(fd)
    writer.writerow(get_field_names(event))
    examples_row = [EXAMPLE_DATA.get(name, '') for name in get_field_names(event)]
...
def validate_row(event, row):
    ...
    # validate row length
    field_names = get_field_names(event)
...
class ActiveCSVImportHandler(base.AuthenticatedHandler):
    ...
    def AuthenticatedPost(self, org, event):
        ...
        if action == 'download_invalid_csv':
            ...
            csv_file_obj = CSVFile.get_by_id(int(csv_id))
            rows_for_csv = ( 
                [['ERRORS: !!remove this column!!'] + get_field_names(event)] + 
