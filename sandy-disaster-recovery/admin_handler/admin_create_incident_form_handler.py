...
"""
	...
	incident_csv_db.CheckDuplicatesAndPut(incident_csv = i, cache_time = cache_time, event = incident)
	...
	form_db.CheckDuplicatesAndPut(form_html = f, cache_time = cache_time, event=incident)
...
class HtmlPropertiesParser(HTMLParser.HTMLParser):
	...
	for name, value in attrs:
	  if name == "id":
	    ...
	    # need to find the type... really only need select and checkbox
	  if name == "type":
	    PROPERTIES_TYPES_LIST.append(value)
	    counter += 1
